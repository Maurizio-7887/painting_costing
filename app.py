...
# ══════════════════════════════════════════════════════════════════
# ROUTE: CARICO PDF  (Import ordini da PDF NetPro/SAP)
# ══════════════════════════════════════════════════════════════════

MACCHINE_CONFIG = {
    'VERN-01': {'desc': 'Tunnel principale 170m', 'ganci_tot': 425, 'vel_default': 1.5, 'tipo': 'tunnel'},
    'VERN-02': {'desc': 'Tunnel piccoli pezzi 90m', 'ganci_tot': 220, 'vel_default': 2.0, 'tipo': 'tunnel'},
    'VERN-03': {'desc': 'Cabina manuale grandi strutture', 'ganci_tot': 80, 'vel_default': 0, 'tipo': 'cabina'},
}

def parse_netpro_pdf(file_bytes):
    """Estrae dati da PDF NetPro/SAP. Ritorna dict con header + righe."""
    import pdfplumber, io, re
    result = {'macchina': '', 'centro': '', 'data': '', 'operatore': '',
              'doc_num': '', 'colore': '', 'ganci_tot': 0, 'velocita': '',
              'righe': [], 'errori': []}
    try:
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for page in pdf.pages:
                ...
                # Estrai header
                m = re.search(r'Centro:\s*(VERN-\w+)', text)
                if m: result['centro'] = m.group(1)
                m = re.search(r'Macchina:\s*(VERN-\w+)', text)
                if m: result['macchina'] = m.group(1)
                m = re.search(r'Data produzione:\s*([\d/]+)', text)
                if m: result['data'] = m.group(1)
                m = re.search(r'Operatore:\s*([^\|]+)', text)
                if m: result['operatore'] = m.group(1).strip()
                m = re.search(r'Documento:\s*(NP-[\w-]+)', text)
                if m: result['doc_num'] = m.group(1)
                m = re.search(r'Colore:\s*([^\|]+)', text)
                if m: result['colore'] = m.group(1).strip()
                m = re.search(r'Ganci totali:\s*(\d+)', text)
                if m: result['ganci_tot'] = int(m.group(1))
                m = re.search(r'Velocità catena:\s*([\d.]+)', text)
                if m: result['velocita'] = m.group(1)
                ...
                for table in tables:
                    for row in table:
                        ...
                        if row[0] in ('N.Ord.', '', None): continue
                        if row[2] and 'TOTALE' in str(row[2]): continue
                        try:
                            ordine = str(row[0]).strip()
                            codice = str(row[1]).strip()
                            desc   = str(row[2]).strip()
                            fam    = str(row[3]).strip() if row[3] else ''
                            qty    = int(str(row[4]).strip()) if row[4] else 1
                            peso_s = str(row[5]).replace('kg','').strip() if row[5] else '0'
                            peso   = float(peso_s) if peso_s else 0.0
                            ganci  = int(str(row[6]).strip()) if row[6] else 1
                            colore = str(row[7]).strip() if row[7] else result['colore']
                            prio   = int(str(row[8]).strip()) if row[8] else 5
                            note   = str(row[9]).strip() if len(row) > 9 and row[9] else ''
                            if codice and ordine.isdigit():
                                result['righe'].append({
                                    'ordine': ordine, 'codice': codice,
                                    'descrizione': desc, 'famiglia': fam,
                                    'qty': qty, 'peso': peso, 'ganci': ganci,
                                    'colore': colore, 'prio': prio, 'note': note
                                })
                        except Exception as e:
                            result['errori'].append(f'Riga saltata: {e}')
    except Exception as e:
        result['errori'].append(f'Errore parsing PDF: {e}')
    return result


def ottimizza_per_macchina(righe, macchina_cfg):
    """Bin-packing: massimizza pezzi nel minor numero di ganci possibile."""
    ganci_tot = macchina_cfg.get('ganci_tot', 425)
    # Ordina: prima grandi (più ganci), poi piccoli → migliore packing
    righe_ord = sorted(righe, key=lambda r: (-r['ganci'], -r['peso']))
    ganci_usati = 0
    allocati = []
    non_allocati = []
    for r in righe_ord:
        g_riga = r['ganci'] * r['qty']
        if ganci_usati + g_riga <= ganci_tot:
            ganci_usati += g_riga
            allocati.append(r)
        else:
            # Prova ad allocare parzialmente
            spazio = ganci_tot - ganci_usati
            pezzi_max = spazio // r['ganci']
            if pezzi_max > 0:
                r_parz = dict(r); r_parz['qty'] = pezzi_max; r_parz['_parziale'] = True
                allocati.append(r_parz)
                ganci_usati += pezzi_max * r['ganci']
                r_rest = dict(r); r_rest['qty'] = r['qty'] - pezzi_max
                non_allocati.append(r_rest)
            else:
                non_allocati.append(r)
    pezzi_tot = sum(r['qty'] for r in allocati)
    peso_tot = sum(r['qty'] * r['peso'] for r in allocati)
    sat_pct = round(ganci_usati / ganci_tot * 100, 1) if ganci_tot else 0
    return {
        'allocati': allocati, 'non_allocati': non_allocati,
        'ganci_usati': ganci_usati, 'ganci_tot': ganci_tot,
        'pezzi_tot': pezzi_tot, 'peso_tot': round(peso_tot, 1),
        'sat_pct': sat_pct,
    }


@app.route('/carico_pdf', methods=['GET', 'POST'])
def carico_pdf():
    parsed = None
    ottimizzato = None
    saved_lotto = None
    if request.method == 'POST':
        ...
        if action == 'parse':
            ...
            if f and f.filename.lower().endswith('.pdf'):
                file_bytes = f.read()
                parsed = parse_netpro_pdf(file_bytes)
                if parsed['macchina'] in MACCHINE_CONFIG:
                    mac_cfg = MACCHINE_CONFIG[parsed['macchina']]
                    ottimizzato = ottimizza_per_macchina(parsed['righe'], mac_cfg)
                else:
                    flash(f"Macchina '{parsed['macchina']}' non trovata in configurazione.", 'warning')
            else:
                flash('Carica un file PDF valido.', 'danger')
        elif action == 'salva':
            # Ricostruisce dati da form e salva come Lotto
            macchina = request.form.get('macchina', 'VERN-01')
            operatore = request.form.get('operatore', '')
            data_prod = request.form.get('data_prod', date.today().isoformat())
            doc_num = request.form.get('doc_num', '')
            ganci_usati = int(request.form.get('ganci_usati', 0))
            ganci_tot = int(request.form.get('ganci_tot', 425))
            pezzi_tot = int(request.form.get('pezzi_tot', 0))
            peso_tot = float(request.form.get('peso_tot', 0))
            sat_pct = float(request.form.get('sat_pct', 0))
            vel = float(request.form.get('velocita', 1.5) or 1.5)

            cfg = get_config()
            lotto = Lotto(
                codice_lotto = f"PDF-{macchina}-{date.today().strftime('%Y%m%d')}",
                nome = f"Carico PDF {macchina} – {doc_num}",
                ...
                n_pezzi_totali = pezzi_tot,
                n_ganci_totali = ganci_usati,
                lunghezza_usata_m = round(ganci_usati * 0.4, 1),
                completato_at = datetime.now(timezone.utc),
                note = f"Importato da PDF NetPro – doc {doc_num} – macchina {macchina}",
            )
            # Calcola costo grezzo
            mac_cfg = MACCHINE_CONFIG.get(macchina, {})
            t_ciclo_h = (cfg.lunghezza_tunnel_m / max(vel, 0.1)) / 60 if vel > 0 else 1.0
            lotto.costo_totale_eur = round(t_ciclo_h * cfg.costo_orario_esercizio, 2)
            lotto.costo_totale = lotto.costo_totale_eur
            db.session.add(lotto)
            db.session.flush()

            # Salva items
            codici_raw = request.form.get('codici_json', '[]')
            try:
                codici = json.loads(codici_raw)
            except Exception:
                codici = []
            for c in codici:
                prod = Prodotto.query.filter_by(codice=c.get('codice','')).first()
                it = LottoItem(
                    lotto_id = lotto.id,
                    prodotto_id = prod.id if prod else None,
                    quantita = c.get('qty', 1),
                    n_ganci_occupati = c.get('ganci', 1) * c.get('qty', 1),
                )
                db.session.add(it)

            db.session.commit()
            flash(f'✅ Lotto PDF salvato! {pezzi_tot} pezzi · {ganci_usati} ganci · {sat_pct}% saturazione', 'success')
            return redirect(url_for('lotto_detail', lotto_id=lotto.id))

    return render_template('carico_pdf.html',
                           parsed=parsed, ottimizzato=ottimizzato,
                           macchine=MACCHINE_CONFIG)


# ══════════════════════════════════════════════════════════════════
# API: Dashboard WOW stats
# ══════════════════════════════════════════════════════════════════
...
def api_dashboard_stats():
    ...
    lotti_oggi = Lotto.query.filter(
        Lotto.completato_at >= datetime.combine(oggi, datetime.min.time(), tzinfo=timezone.utc)
    ).all() if hasattr(datetime, 'combine') else []
    ...
    try:
        ...
        ganci_oggi = sum(l.n_ganci_totali or 0 for l in lotti_oggi)
    except Exception:
        pezzi_oggi = costo_oggi = ganci_oggi = 0
        lotti_oggi = []

    # Saturazione media ultima settimana
    lotti_sett = Lotto.query.filter(
        Lotto.stato == 'completato'
    ).order_by(Lotto.id.desc()).limit(20).all()
    sat_media = round(sum(l.saturazione_pct or 0 for l in lotti_sett) / max(len(lotti_sett), 1), 1)

    # Ordini in attesa per macchina
    ordini_per_mac = {}
    for mac in MACCHINE_CONFIG:
        cnt = OrdineOrdine.query.filter_by(stato='ATTESA', centro_lavoro=mac).count()
        ordini_per_mac[mac] = cnt
    ...
    for i in range(6, -1, -1):
        ...
        try:
            ...
            s = dt2.combine(d, dt2.min.time()).replace(tzinfo=timezone.utc)
            e = dt2.combine(d, dt2.max.time()).replace(tzinfo=timezone.utc)
            ll = Lotto.query.filter(
                Lotto.completato_at >= s,
                Lotto.completato_at <= e,
                Lotto.stato == 'completato'
            ).all()
    ...
    return jsonify({
        ...
        'ordini_per_mac': ordini_per_mac,
        'trend': trend,
        'macchine': {k: v['desc'] for k, v in MACCHINE_CONFIG.items()},
    })
